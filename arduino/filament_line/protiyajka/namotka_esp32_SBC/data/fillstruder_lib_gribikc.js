...
class read_sys_info{
	constructor(param){
		this.param=param;
		new periodic_shot_gr({url:config['dev_url']+"/help_json", callback:new nt_json_gr(this) });
	}
	
	parser_data(stream){
		this.param.div.innerHTML="";
		//console.log(stream.Uptime);

		let seconds = stream.Uptime;
		...
		//stream.Uptime=days+"Дней " + hours + "Часв " + minutes + "Минут " + seconds + "Секунд";
		stream.Uptime=days+"Д | " + hours + ":" + minutes + ":" + seconds + "";

		//console.log(days,hours,minutes,seconds);

		create_tree_ul_li_from_array_gr(stream,this.param.div,0);
	}
	error_event(message){
		console.log(this);
		console.log(message+'');
...
class get_log_info{
	constructor(param){
		this.param=param;
		new periodic_shot_gr({url:config['dev_url']+"/get_log", callback:this });
	}
	
	parser_data(stream){
		this.param.div.innerHTML=stream.replace(/\n/g,"<br>");;//.replace(/\r\n|\r|\n/g,"<br>");
	}
	error_event(message){
		console.log(this);
		console.log(message+'');
