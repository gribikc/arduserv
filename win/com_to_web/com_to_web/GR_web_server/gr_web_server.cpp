...
void GR_web_server::init() {
  ...
  if (settings.load_settings("GR_web_server", &conf_var)) {
    ...
    if (QSysInfo::productType() == "android") {
      ...
      QStringList sdcommonPaths = {"/storage/86BB-1D02", "/storage/emulated/0",
                                   "/storage/sdcard0/",  "/mnt/sdcard",
                                   "/storage/extSdCard", "/storage/UsbDriveA",
                                   "/storage/UsbDriveB"};
      ...
      conf_var["htdocs_patch"] =
          dir.currentPath(); // conf_var["htdocs_patch"]="/storage/emulated/0/com_to_web";///htdocs/
...
void GR_web_server::gr_sock_srv_start() {
  ...
  server->listen(QHostAddress::Any,
                 conf_var["tcp_listen_port"].toInt()); // 3128

  webs_server =
      new QWebSocketServer("CTW", QWebSocketServer::NonSecureMode, this);
  webs_server->listen(QHostAddress::Any, conf_var["wbs_listen_port"].toInt());
  connect(webs_server, &QWebSocketServer::newConnection, this,
          &GR_web_server::onNewWebs_connect);
  ...
  if (server->isListening()) {
    ...
    connect(server, &QTcpServer::newConnection, this,
            &GR_web_server::incommingConnection);
  } else {
...
void GR_web_server::onNewWebs_connect() {
  ...
  connect(abv, &GR_http_client::dataComplete, this,
          &GR_web_server::client_requestComplete);
  abv->init(socket);
...
void GR_web_server::client_requestComplete(GR_http_client *http_client) {
  ...
  for (int i = 0; i < sub_requests.size(); i++) {
    if ((sub_requests[i].type == ComparisonType::StartsWith &&
         http_client->is_rsw(sub_requests[i].str) > 0) || //!!!???==1?
        (sub_requests[i].type == ComparisonType::Matches &&
         http_client->is_rsw(sub_requests[i].str) == 2)) {
  ...
  //Идем дальше если нет зарегистрированных событий
  ///////////////
  ...
  //Если нет запрашиваемого файла идем дальше
  ///////////////
  // 404
  // http_client->send_html_header();
  ...
  http_client->socket->write("<a href=\"/sys/tree\">/sys/tree/(h,j,r,...)</a> "
                             "-Дерево HtDocs               <br>\n");
  http_client->socket->write(
      "<a href=\"/sys/settings/edit.html\">/sys/settings</a>                   "
      "    <br>\n");
  http_client->socket->write(
      "<a href=\"/dev/com/l\">/dev/com/(w,r,s,l)/(num)/(speed)/</a>   <br>\n");
  http_client->socket->write("/dev/sens/(w,r,s)/(type)/           <br>\n");
  http_client->socket->write(
      "<a href=\"/dev/bt/l\">/dev/bt/(w,r,s,l)/(name)</a>            <br>\n");
  http_client->socket->write(
      "<a href=\"/dev/gps/r\">/dev/gps/(w,r,s)/</a>                   <br>\n");
...
void GR_web_server::registaration_sys() {
  reg_on("/sys/tree", StartsWith, SingleShot, HTMLHeader,
         [&](GR_http_client *http_client) {
           QDir dir;
           get_tree_file(conf_var["htdocs_patch"].toString() + "\\htdocs", "",
                         http_client, conf_var["htdocs_patch"].toString());
           GR_logger::log(this, "CtW Htdocs Tree");
         });
  reg_on("/sys/settings", StartsWith, SingleShot, HTMLHeader,
         [&](GR_http_client *http_client) {
           if (http_client->is_rsw("/sys/settings/c") > 0) {
             ...
             conf_var.clear();
             conf_var.operator=(
                 settings.create_arr_from_json(http_client->indata));
             ...
             http_client->socket->write(
                 settings.create_json_from_arr(conf_var).toLocal8Bit());
           }
           GR_logger::log(this, "CtW GET Log");
         });
  reg_on("/sys/log", StartsWith, SingleShot, HTMLHeader,
         [&](GR_http_client *http_client) {
  ...
  reg_on(
      "/", Matches, SingleShot, FileHeader, [&](GR_http_client *http_client) {
  ...
  reg_on("/favicon.ico", Matches, SingleShot, HTMLHeader,
         [&](GR_http_client *http_client) {
           http_client->socket->write("Nice try to get favicon.ico :)))");
           GR_logger::log(this, "CtW TTG favicon.ico");
         });
  reg_on("/htdocs", StartsWith, SingleShot, FileHeader,
         [&](GR_http_client *http_client) { //!!! убрать
           QStringList list_param = http_client->get_list_param();
  ...
  reg_on("/db/w", StartsWith, SingleShot, HTMLHeader,
         [&](GR_http_client *http_client) {
